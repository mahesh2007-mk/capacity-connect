...
def get_db_path():
    ...
    if os.environ.get("VERCEL"):
        vercel_tmp_path = "/tmp/capacity_connect.db"
        if not os.path.exists(vercel_tmp_path):
            repo_db = os.path.join(_parent_dir, "capacity_connect.db")
            if os.path.exists(repo_db):
                try:
                    import shutil
                    shutil.copy2(repo_db, vercel_tmp_path)
                except Exception:
                    pass
    ...
    if custom_path:
        if not os.path.isabs(custom_path):
            return os.path.normpath(os.path.join(_parent_dir, custom_path))
        return custom_path
    return os.path.normpath(os.path.join(_parent_dir, "capacity_connect.db"))
