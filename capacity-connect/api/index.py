...
from api.ai_bot import get_assessment_question_set
...
def get_trainee_learning_path(course_id: int, user: dict = Depends(require_role(["trainee"]))):
    ...
    for mod in modules:
        ...
        # Fetch lessons with YouTube video IDs
        cur.execute("SELECT * FROM lessons WHERE module_id = ? ORDER BY lesson_number ASC", (mod["id"],))
        mod["lessons"] = [dict(l) for l in cur.fetchall()]
...
def create_course(req: CourseCreate, user: dict = Depends(require_role(["admin"]))):
    ...
    modules_to_create = req.modules if req.modules else [
        ModuleCreate(title="Module 1: Foundations & Architecture", description="Core introductory concepts and design primitives", lessons=[
            LessonCreate(title="Introduction & Overview", content="Foundational overview of concepts", video_url="https://www.youtube.com/watch?v=kqtD5dpn9C8", video_id="kqtD5dpn9C8")
        ]),
        ModuleCreate(title="Module 2: In-Depth Implementation", description="Hands-on coding, patterns, and practical configurations", lessons=[
            LessonCreate(title="Applied Techniques", content="Practical implementation techniques", video_url="https://www.youtube.com/watch?v=bMknfKXIFA8", video_id="bMknfKXIFA8")
        ]),
        ModuleCreate(title="Module 3: Advanced Optimization", description="Performance profiling, edge cases, and design constraints", lessons=[
            LessonCreate(title="Performance & Scaling", content="Optimization strategies and profiling", video_url="https://www.youtube.com/watch?v=inWWhr5tnEA", video_id="inWWhr5tnEA")
        ]),
        ModuleCreate(title="Module 4: Enterprise Production Deployment", description="Real-world system integration, security, and verification", lessons=[
            LessonCreate(title="Production Readiness", content="Hardening and deployment pipelines", video_url="https://www.youtube.com/watch?v=pkYVOmU3MgA", video_id="pkYVOmU3MgA")
        ])
    ]
    ...
    for m_idx, mod in enumerate(modules_to_create, start=1):
        ...
        for l_idx, les in enumerate(mod.lessons, start=1):
            cur.execute("""
                INSERT INTO lessons (module_id, lesson_number, title, content, video_url, video_id, duration_minutes)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (mod_id, l_idx, les.title, les.content, les.video_url, les.video_id, les.duration_minutes))

    # Add 20 default questions for assessment bank
    sample_questions = [
        (f"What is a primary architectural principle taught in {req.title}?", "Modularity & High Cohesion", "Random Logic", "Hardcoding Keys", "Disabling Security", "A"),
        (f"Which method improves resilience and reliability in {req.subject}?", "Robust Exception Trapping & Logging", "Ignoring Errors", "Infinite Loops", "Turning off Backups", "A"),
        (f"How do we achieve scalability according to {req.title}?", "Horizontal Partitioning & Stateless Services", "Single Point of Failure", "Global Locking", "Manual Restarts", "A"),
        (f"What is the recommended testing approach for {req.subject}?", "Automated Unit and Integration Testing", "Test in Production Only", "No Tests", "Rely on User Complaints", "A"),
    ]
    for i in range(20):
        template = sample_questions[i % len(sample_questions)]
        cur.execute("""
            INSERT INTO questions (course_id, question_text, option_a, option_b, option_c, option_d, correct_answer, explanation)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (course_id, f"Question {i+1}: {template[0]}", template[1], template[2], template[3], template[4], template[5], "Standard curriculum answer based on module principles."))
